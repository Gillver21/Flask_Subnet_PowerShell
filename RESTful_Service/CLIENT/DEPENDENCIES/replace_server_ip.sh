...
sudo find .. -type f -name "*.sh" -not -name "replace*" -exec sed -i -E "s/http:\/\/[0-9]+.[0-9]+.[0-9]+.[0-9]+:[0-9]+/$IP_PORT_AWS/g" {} + > /dev/null 2>&1
find .. -type f -name "*.sh" -not -name "replace*" -exec sed -i -E "s/http:\/\/[0-9]+.[0-9]+.[0-9]+.[0-9]+:[0-9]+/$IP_PORT_AWS/g" {} +
